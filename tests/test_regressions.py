...
"""
    ...
    def test_markdown_quality_note_backfill_dry_run_then_apply(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path = Path(tmpdir)
            db_path = tmp_path / "meetings.db"
            output_path = tmp_path / "legacy-markdown-repeat.md"
            repeated_lines = "\n".join(
                f"[70:{index:02d}] **[發言者 A]**：因為我是結所以我領車。"
                for index in range(5)
            )
            original_markdown = (
                ...
                "| # | 關聯討論 | 決議 | 依據 | 狀態 |\n"
                "|---|---|---|---|---|\n"
                "| R1 | D1 | 確認需回填 Markdown 提示。 | [70:00] | confirmed |\n\n"
                "## 三、待辦事項 (Action Items)\n\n"
                "| # | 關聯討論 | 關聯決議 | 任務描述 | 負責人 | 期限 | 優先級 |\n"
                "|---|---|---|---|---|---|---|\n"
                "| A1 | D1 | R1 | 複核問題分段。 | 發言者 A | 未提及 | 中 |\n\n"
...
database.init_db()
database.save_meeting(
    title='Legacy Markdown Repeat Warning',
    date='2026/07/20',
    source_audio='legacy-markdown-repeat.webm',
    output_path={json.dumps(str(output_path))},
    summary='Legacy Markdown Repeat Warning',
    quality_report={{
        'score': 95,
        'label': '良好',
        'warnings': [
...
"""
            ...
            dry_run = subprocess.run(
                [sys.executable, "scripts/backfill_markdown_quality_notes.py"],
                cwd=ROOT,
                env=env,
                check=False,
                capture_output=True,
            ...
            self.assertEqual(dry_payload["would_update"], 1)
            self.assertEqual(dry_payload["updated"], 0)
            self.assertIn("第 8 段", dry_payload["records"][0]["review_summary"])
            self.assertEqual(output_path.read_text(encoding="utf-8"), original_markdown)

            applied = subprocess.run(
                [sys.executable, "scripts/backfill_markdown_quality_notes.py", "--apply"],
                cwd=ROOT,
                env=env,
                check=False,
                capture_output=True,
            ...
            updated_markdown = output_path.read_text(encoding="utf-8")
            self.assertIn("逐字稿品質複核提示", updated_markdown)
            self.assertIn("第 8 段 70:00-80:00", updated_markdown)
            self.assertIn("同一句連續重複 5 次", updated_markdown)
            ...
            self.assertEqual(second_dry_run.returncode, 0, second_dry_run.stdout + second_dry_run.stderr)
            second_payload = json.loads(second_dry_run.stdout)
            self.assertEqual(second_payload["would_update"], 0)
            self.assertEqual(updated_markdown.count("逐字稿品質複核提示"), 1)
...
class TaskRegressionTests(unittest.TestCase):
    ...
    def test_segment_speaker_context_never_carries_prior_utterance_text(self):
        from backend import tasks

        prior_transcript = (
            "[29:10] **[發言者 A]**：好，那 100 的時候，上限 920，下限 782。\n"
            "[29:16] **[發言者 B]**：對。"
        )

        context = tasks._speaker_context_from_transcripts([prior_transcript])
        prompt = tasks._build_segment_prompt(3, 6, speaker_context=context)

        self.assertIn("發言者 A", context)
        self.assertIn("發言者 B", context)
        self.assertNotIn("上限 920", context)
    ...
    def test_old_record_reuse_blocks_hallucination_and_missing_timestamps_but_allows_sparse_legacy_timestamps(self):
        ...
        structured = "\n".join(
            (
                f"[20:{index * 4:02d}] **[發言者 A]**："
                f"項目 {31 + index}，上限 {230 + index * 10}，下限 {195.5 + index * 8.5:.1f}。"
            )
        ...
        allowed = _record_segment_reuse_blocking_issues(
            sparse_legacy,
            segment_index=2,
            total_segments=6,
            expected_start_seconds=1198,
    ...
    def test_recovery_revalidates_combined_children_for_structured_hallucination(self):
        ...
        with mock.patch.object(
            tasks,
            "_transcribe_segment",
            side_effect=[incomplete, numeric_chunk(1), numeric_chunk(7)],
        ), mock.patch.object(
            tasks,
            "_split_audio_to_subsegments",
            return_value=[(sub1, 0, 300), (sub2, 300, 600)],
        ), mock.patch.object(tasks, "is_job_cancel_requested", return_value=False), mock.patch.object(
            tasks, "update_job_status"
        ):
            with self.assertRaisesRegex(RuntimeError, "數列延伸轉錄幻覺"):
                tasks._transcribe_segment_with_recovery(
                    client=object(),
                    seg_path=Path("segment.mp3"),
                    seg_index=0,
                    total_segs=2,
                    job_id="combined-recovery-job",
                    model="gemini-test",
                    offset_seconds=0,
    ...
    def test_quality_report_score_reflects_multiple_review_segments(self):
        ...
        self.assertEqual(report["label"], "可用，建議抽查")
        self.assertEqual([segment["index"] for segment in report["review_segments"]], [0, 1, 2])
...
class MetricsRegressionTests(unittest.TestCase):
    ...
    def test_metrics_endpoint_reports_storage_usage(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (source_dir / "_seg_meeting-a_000.mp3").write_bytes(b"temporary-segment")
            (source_dir / "_sub__seg_meeting-a_000_300s_000.mp3").write_bytes(b"temporary-subsegment")
            ...
            with mock.patch.object(main, "SOURCE_AUDIO_DIR", source_dir), \
                ...
                temp_segment_media_response = asgi_request(main.app, "GET", "/source-media/inventory/_seg_meeting-a_000.mp3")
                temp_segment_delete_response = asgi_request(main.app, "DELETE", "/source-media/inventory/_seg_meeting-a_000.mp3")
        ...
        self.assertNotIn("_seg_meeting-a_000.mp3", [item["name"] for item in inventory["files"]])
        self.assertNotIn("_sub__seg_meeting-a_000_300s_000.mp3", [item["name"] for item in inventory["files"]])
        ...
        self.assertEqual(temp_segment_media_response.status_code, 400)
        self.assertIn("暫存分段", temp_segment_media_response.json()["detail"])
        self.assertEqual(temp_segment_delete_response.status_code, 400)
        self.assertIn("暫存分段", temp_segment_delete_response.json()["detail"])
...
class SearchRegressionTests(unittest.TestCase):
    ...
    def test_list_and_search_include_review_segment_labels(self):
        ...
        quality_report = {
            ...
            "review_segments": [
                {"index": 3, "start_seconds": 1800, "end_seconds": 2400, "issues": ["時間戳不連續"]},
                {"index": 1, "label": "第 2 段", "start_seconds": 600, "end_seconds": 1200, "issues": ["疑似連續重複轉錄"]},
            ],
            "segments": [
                {"index": 3, "issues": []},
...
""".strip(),
    ...
    def test_list_and_search_infer_review_segments_from_warning_text(self):
        ...
        output_path.write_text("warning-segments-content", encoding="utf-8")
        quality_report = {
            "score": 88,
            "label": "需複核",
            "warnings": [
        ...
        meeting_id = database.save_meeting(
            title="Warning Segment Labels",
            date="2026/07/12",
            source_audio="warning-segments.webm",
            output_path=str(output_path),
            summary="warning-segments-summary",
            quality_report=quality_report,
        )

        listed = next(row for row in database.list_meetings() if row["id"] == meeting_id)
        searched = database.search_meetings("Warning Segment Labels")[0]
    ...
    def test_list_and_search_locate_repeat_warning_by_phrase_when_loop_detector_misses(self):
        database, tmp_path = self._isolated_database()
        output_path = tmp_path / "phrase-located-repeat-warning.md"
        output_path.write_text(
            "## 一、討論摘要 (Discussion Summary)\n摘要\n"
            "## 二、最終決議 (Final Decisions)\n決議\n"
            "## 三、待辦事項 (Action Items)\n| # | 任務描述 | 負責人 | 期限 | 優先級 |\n"
            ...
            "## 📝 四、完整逐字稿 (Verbatim Transcript)\n"
            "### 【第 3 段｜20:00 – 30:00】\n"
            "[24:12] **[發言者 A]**：因為我是結，所以我領車，這句話被品質警示指出。\n"
            "[24:20] **[發言者 B]**：後續正常討論。\n",
            encoding="utf-8",
        )
        quality_report = {
        ...
        meeting_id = database.save_meeting(
            title="Phrase Located Repeat Warning",
            date="2026/07/20",
            source_audio="phrase-located-repeat.webm",
            output_path=str(output_path),
            summary="phrase-located-repeat-summary",
            quality_report=quality_report,
        )

        listed = next(row for row in database.list_meetings() if row["id"] == meeting_id)
        searched = database.search_meetings("Phrase Located Repeat Warning")[0]

        expected_preview = (
            "逐字稿品質警示：問題位置：第 3 段 20:00-30:00："
            "疑似連續重複轉錄；同一句連續重複 31 次：因為我是結所以我領車；重複時間：24:12-24:12"
        )
        self.assertEqual(listed["quality_review_segments"], ["第 3 段"])
        self.assertEqual(listed["quality_review_segment_count"], 1)
        self.assertEqual(listed["quality_review_rerunnable_segments"], [2])
        self.assertEqual(listed["quality_warning_preview"], expected_preview)
        self.assertEqual(
            listed["quality_warning_text"].splitlines()[0],
            expected_preview + "。建議重跑上述分段或複核相關內容。",
    ...
    def test_list_and_search_infer_repeat_segments_from_generic_warning(self):
        database, tmp_path = self._isolated_database()
        output_path = tmp_path / "generic-warning-repeat.md"
        repeated_turns = "\n".join(
            f"[21:0{index}] **[發言者 A]**：因為我是結所以我領車。"
            for index in range(4)
        )
        output_path.write_text(
            "## 一、討論摘要 (Discussion Summary)\n摘要\n"
        ...
        quality_report = {
            "warnings": ["舊紀錄需複核：早期測試未建立分段定位"],
        }
        meeting_id = database.save_meeting(
            title="Generic Warning Repeat",
            date="2026/07/12",
            source_audio="generic-warning-repeat.webm",
            output_path=str(output_path),
            summary="generic-warning-repeat-summary",
            quality_report=quality_report,
        )
        ...
        self.assertEqual(searched["quality_review_segments"], ["第 3 段"])
        self.assertEqual(searched["quality_review_segment_details"], listed["quality_review_segment_details"])
...
"""
    ...
    def test_web_ui_has_job_dashboard(self):
        ...
        self.assertIn('id="job-event-panel" role="region" aria-labelledby="job-event-heading" aria-live="polite" aria-hidden="true"', html)
        self.assertIn('id="job-event-list" role="list" aria-label="任務事件清單" aria-busy="false"', html)
        ...
        self.assertIn('class="job-event" role="listitem"', html)
    ...
    def test_web_ui_has_metrics_panel_and_job_actions(self):
        ...
        self.assertIn('id="source-storage-preview" class="source-storage-preview" role="region" aria-label="原始檔預覽" aria-live="polite" tabindex="-1"', html)
        ...
        self.assertIn('class="source-storage-item" role="listitem"', html)
        ...
        self.assertIn("let sourceMediaPreviewReturnFocus = null", html)
        self.assertIn("function closeSourceMediaPreview(restoreFocus = true)", html)
        self.assertIn("sourceMediaPreviewReturnFocus = activeElement;", html)
        self.assertIn("!preview.contains(activeElement)", html)
        self.assertIn("preview.focus({ preventScroll: true });", html)
        self.assertIn("returnTarget.focus();", html)
        self.assertIn("function setSourceStorageStatus", html)
        self.assertIn("function setSourceStorageStatus(message = '', isError = false, busy = false)", html)
        self.assertIn("status.classList.toggle('loading', Boolean(busy) && Boolean(message));", html)
        ...
        self.assertIn("setElementBusy(list, true)", html)
        self.assertIn("setElementBusy(list, false)", html)
        self.assertIn("button.setAttribute('aria-busy', 'true');", html)
        self.assertIn("button.setAttribute('aria-busy', 'false');", html)
        self.assertIn("refresh.setAttribute('aria-busy', 'true');", html)
        ...
        self.assertIn("closeSourceMediaPreview(false);\n  closeAccessibleModal(document.getElementById('source-storage-modal'))", html)
        ...
        self.assertIn("setSourceStorageStatus('正在更新原始檔清單...', false, true);", html)
        ...
        self.assertIn("const VIDEO_FILE_EXTENSIONS = new Set(['.webm', '.mp4', '.mov', '.mkv', '.avi', '.mpeg', '.mpg', '.wmv'])", html)
        self.assertIn("const AUDIO_FILE_EXTENSIONS = new Set(['.mp3', '.wav', '.m4a', '.aac', '.ogg', '.opus', '.flac', '.wma'])", html)
        ...
        self.assertIn("source-media-mode-pill", html)
        self.assertIn("function sourceMediaModePill", html)
        self.assertIn("function sourceMediaModeFacts", html)
        self.assertIn("function mediaPlayerCurrentSeconds", html)
        self.assertIn("function mediaPlayerPlaybackSnapshot", html)
        self.assertIn("function restoreMediaPlayerPlayback", html)
        self.assertIn("function restoreMediaPlayerSeconds", html)
        self.assertIn("const previousPlayer = preview.querySelector('video.source-storage-media-player, audio');", html)
        self.assertIn("const previousPlayback = mediaPlayerPlaybackSnapshot(previousPlayer);", html)
        self.assertIn("restoreMediaPlayerPlayback(preview.querySelector('video.source-storage-media-player, audio'), previousPlayback);", html)
        self.assertIn("wasPlaying: Boolean(player && !player.paused && !player.ended)", html)
        self.assertIn("Unable to resume source media playback after switch.", html)
        self.assertIn("影片預覽中", html)
        self.assertIn("音訊預覽中", html)
        self.assertIn("sourceMediaModeFacts(playerMode)", html)
        ...
        self.assertIn('role="status" aria-live="polite" aria-atomic="true" aria-busy="false"', html)
        ...
        self.assertIn("source-media-mode-action", html)
        self.assertIn('aria-pressed="${playerMode === \'video\'}"', html)
        self.assertIn('aria-pressed="${playerMode === \'audio\'}"', html)
        self.assertIn("目前為影片預覽", html)
        self.assertIn("目前為音訊預覽", html)
        self.assertIn("切換為音訊預覽", html)
        self.assertIn("切換為影片預覽", html)
        self.assertIn("🔊 音訊", html)
        self.assertIn("🎥 影片", html)
    ...
    def test_web_ui_loads_runtime_config_and_prevents_oversized_uploads(self):
        ...
        self.assertIn('id="progress-box" role="status" aria-live="polite" aria-atomic="true" aria-busy="false"', html)
        self.assertIn('aria-label="關閉上傳視窗"', html)
        self.assertIn("function openUploadFilePicker", html)
        self.assertIn("function handleDropZoneKeydown", html)
        self.assertIn("dropZone.addEventListener('keydown', handleDropZoneKeydown)", html)
        self.assertIn("e.key !== 'Enter' && e.key !== ' '", html)
        self.assertIn(".upload-area:focus-visible", html)
        self.assertIn('.upload-area[aria-disabled="true"]', html)
        self.assertIn("box-shadow: 0 0 0 3px rgba(122,162,247,0.18);", html)
        ...
        self.assertIn("progressBox?.setAttribute('aria-busy', String(uploadBusy));", html)
        self.assertIn("dropZone?.setAttribute('aria-disabled', String(uploadBusy));", html)
        self.assertIn("if (uploadBusy) return;", html)
    ...
    def test_web_ui_has_readable_api_errors_and_job_event_timeline(self):
        ...
        self.assertIn('aria-label="關閉任務事件"', html)
        self.assertIn("panel.setAttribute('aria-hidden', 'false')", html)
        self.assertIn("panel.setAttribute('aria-hidden', 'true')", html)
        self.assertIn("setElementBusy(list, true)", html)
        self.assertIn("setElementBusy(list, false)", html)
    ...
    def test_web_ui_can_rerun_a_meeting_from_detail_view(self):
        ...
        self.assertIn('id="rerun-meeting-button" class="btn-secondary" aria-describedby="detail-status" aria-busy="false"', html)
        self.assertIn('id="rerun-summary-button" class="btn-secondary" aria-describedby="detail-status" aria-busy="false"', html)
        self.assertIn('id="rerun-summary-high-quality-button" class="btn-secondary" aria-describedby="detail-status" aria-busy="false"', html)
    ...
    def test_web_ui_has_wide_reading_mode_for_meeting_detail(self):
        ...
        self.assertIn('class="detail-panel" id="detail-panel" aria-label="會議詳情" aria-busy="false"', html)
        self.assertIn('id="detail-status" class="detail-status sr-only" role="status" aria-live="polite" aria-atomic="true"', html)
        self.assertIn('class="detail-content" id="detail-content"', html)
        self.assertIn(".sr-only", html)
        self.assertIn(".detail-content", html)
        ...
        self.assertIn("function detailContentElement", html)
        self.assertIn("function setDetailStatus", html)
        self.assertIn("function setDetailBusy", html)
        self.assertIn("setDetailBusy(true)", html)
        self.assertIn("setDetailBusy(false)", html)
        self.assertIn("setDetailStatus('載入會議記錄中...')", html)
        self.assertIn("setDetailStatus(`已載入會議記錄：${r.title}`)", html)
        self.assertIn("setDetailStatus(`無法載入會議詳情：${err.message}`)", html)
        self.assertIn("setDetailStatus('已清空會議詳情')", html)
        self.assertIn("detailContent.innerHTML", html)
        self.assertIn("detailContentElement().innerHTML", html)
    ...
    def test_web_ui_prevents_detail_header_text_stacking(self):
        ...
        self.assertRegex(html, r"\.detail-actions \.btn-primary,\s*\.detail-actions \.btn-secondary \{[^}]*overflow-wrap: anywhere;")
        self.assertRegex(html, r"\.card-quality-chip \{[^}]*min-width: 0;[^}]*max-width: 100%;[^}]*overflow-wrap: anywhere;")
        self.assertRegex(html, r"\.card-quality-chip\.card-quality-action \{[^}]*white-space: normal;")
        self.assertRegex(html, r"\.source-media-fact \{[^}]*min-width: 0;[^}]*max-width: 100%;[^}]*overflow-wrap: anywhere;")
        self.assertIn(".source-storage-preview-header", html)
        self.assertIn(".source-storage-preview-actions", html)
        self.assertIn(".source-storage-preview:focus-visible", html)
        ...
        self.assertRegex(html, r"\.quality-review-segment \{[^}]*align-items: flex-start;")
        self.assertRegex(html, r"\.quality-review-segment-issue \{[^}]*overflow-wrap: anywhere;[^}]*white-space: normal;")
        self.assertRegex(html, r"\.quality-review-segment-issue \{[^}]*display: -webkit-box;[^}]*-webkit-line-clamp: 2;")
        ...
        self.assertIn(".source-storage-status.loading", html)
        self.assertIn(".source-storage-badge.warning", html)
    ...
    def test_web_ui_can_record_screen_audio_with_toggleable_microphone(self):
        ...
        self.assertIn('class="rec-mode-tabs" role="radiogroup" aria-label="錄製模式"', html)
        self.assertIn('id="tab-audio" role="radio" aria-checked="true" tabindex="0"', html)
        self.assertIn('id="tab-video" role="radio" aria-checked="false" tabindex="-1"', html)
        self.assertIn('id="tab-screen" role="radio" aria-checked="false" tabindex="-1"', html)
        self.assertIn('aria-controls="rec-screen-options"', html)
        self.assertIn('onkeydown="handleRecordingModeKeydown(event)"', html)
        self.assertIn('id="rec-screen-options" aria-labelledby="tab-screen" hidden', html)
        self.assertIn('id="rec-screen-options"', html)
        self.assertIn('id="rec-include-mic" checked', html)
        self.assertIn('id="btn-rec-mic"', html)
        self.assertIn('aria-label="關閉錄製視窗"', html)
        self.assertIn('id="rec-size-estimate" aria-live="polite"', html)
        self.assertIn('id="rec-status" role="status" aria-live="polite"', html)
        self.assertIn('id="rec-progress-box" role="status" aria-live="polite" aria-atomic="true" aria-busy="false"', html)
        ...
        self.assertIn("function syncRecordingModeTabs", html)
        self.assertIn("tab.setAttribute('aria-checked', String(selected));", html)
        self.assertIn("tab.tabIndex = selected ? 0 : -1;", html)
        self.assertIn("function handleRecordingModeKeydown", html)
        self.assertIn("event.key === 'ArrowRight' || event.key === 'ArrowDown'", html)
        self.assertIn("event.key === 'ArrowLeft' || event.key === 'ArrowUp'", html)
        self.assertIn("event.key === 'Home'", html)
        self.assertIn("event.key === 'End'", html)
        self.assertIn("switchRecMode(nextMode);", html)
        self.assertIn("document.getElementById(`tab-${nextMode}`)?.focus();", html)
        self.assertIn("if (recScreenOptions) recScreenOptions.hidden = recMode !== 'screen';", html)
        ...
        self.assertIn("function setRecordingProgressBusy", html)
        self.assertIn("recProg?.setAttribute('aria-busy', String(Boolean(busy)));", html)
        self.assertIn("document.getElementById('rec-title').disabled = locked", html)
        self.assertIn('class="form-label" for="rec-title"', html)
        ...
        self.assertIn("setRecordingProgressBusy(false);", html)
...
"""
    ...
    def test_web_ui_modal_focus_and_escape_controls(self):
        ...
        self.assertIn("function getFocusableModalElements", html)
        ...
        self.assertIn("function trapModalFocus", html)
        ...
        self.assertIn("if (event.key !== 'Tab' || !modal) return false;", html)
        self.assertIn("if (!modal.contains(active))", html)
        self.assertIn("if (event.shiftKey && active === first)", html)
        self.assertIn("if (!event.shiftKey && active === last)", html)
        self.assertIn("if (trapModalFocus(event, modal)) return;", html)
        self.assertIn("if (event.key !== 'Escape') return;", html)
        self.assertIn('role="dialog" aria-modal="true" aria-labelledby="upload-modal-title"', html)
        self.assertIn('id="upload-modal-title"', html)
        self.assertIn('role="dialog" aria-modal="true" aria-labelledby="summary-editor-title"', html)
        self.assertIn('id="summary-editor-title"', html)
        self.assertIn('id="summary-editor-status" class="editor-save-status" role="status" aria-live="polite" aria-atomic="true" aria-busy="false"', html)
        self.assertIn('id="save-summary-button" aria-describedby="summary-editor-status" aria-busy="false"', html)
        self.assertIn('role="dialog" aria-modal="true" aria-labelledby="transcript-editor-title"', html)
        self.assertIn('id="transcript-editor-title"', html)
        self.assertIn('id="transcript-editor-status" class="editor-save-status" role="status" aria-live="polite" aria-atomic="true" aria-busy="false"', html)
        self.assertIn('id="save-transcript-button" aria-describedby="transcript-editor-status" aria-busy="false"', html)
        self.assertIn('role="dialog" aria-modal="true" aria-labelledby="revision-modal-title"', html)
        self.assertIn('id="revision-modal-title"', html)
        self.assertIn('id="revision-list" class="revision-list" role="list" aria-label="版本紀錄清單" aria-live="polite" aria-busy="false"', html)
        self.assertIn('role="dialog" aria-modal="true" aria-labelledby="source-storage-modal-title"', html)
        self.assertIn('id="source-storage-modal-title"', html)
        self.assertIn('role="dialog" aria-modal="true" aria-labelledby="record-modal-title"', html)
        self.assertIn('id="record-modal-title"', html)
        ...
        self.assertIn("function setEditorSaveStatus", html)
        self.assertIn("status.classList.toggle('loading', level === 'loading' && Boolean(message));", html)
        self.assertIn("status.setAttribute('aria-busy', String(Boolean(busy)));", html)
        ...
        self.assertIn('class="revision-item" role="listitem"', html)
...
class FreeOptimizationRegressionTests(unittest.TestCase):
    ...
    def test_recovery_subsegments_are_balanced_without_tiny_tail(self):
        from backend import tasks
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("pydub.AudioSegment.from_file", return_value=FakeAudio()):
                subsegments = tasks._split_audio_to_subsegments(audio_path, chunk_seconds=300)
    ...
    def test_failed_localized_repair_does_not_restore_known_bad_transcript(self):
        ...
        summary_payload = {
            "discussion_summary": [{"topic": "補救失敗重轉", "summary": "完成", "evidence_timecodes": ["10:00"]}],
            "final_decisions": [],
            "action_items": [],
        }
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            source_path.write_text(
                ...
                "[00:00] **[發言者 A]**：保留第一段。\n"
                "### 【第 2 段｜10:00 – 20:00】\n"
                "[10:00] **[發言者 B]**：舊稿已確認有遺漏。\n"
                "[19:40] **[發言者 B]**：舊稿收尾。",
                encoding="utf-8",
            )

            def speech_gap_ranges(segment_path, *_args, **_kwargs):
                if segment_path.name.endswith("_001.mp3"):
                    return [{
                        "start_seconds": 660,
                        "end_seconds": 780,
            ...
            with mock.patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}), \
                 ...
                 mock.patch.object(tasks, "_speech_backed_timestamp_gap_quality_ranges", side_effect=speech_gap_ranges), \
                 mock.patch.object(tasks, "_transcript_repetition_repair_ranges", return_value=[]), \
                 mock.patch.object(tasks, "_repair_existing_segment_timestamp_gaps", return_value=(None, [])) as repair_mock, \
                 mock.patch.object(tasks, "_transcribe_segment_with_recovery", return_value=fresh_second) as full_rerun_mock, \
                 mock.patch.object(tasks, "_segment_transcript_current_quality_issues", return_value=[]), \
                 mock.patch.object(tasks, "is_job_cancel_requested", return_value=False), \
                 mock.patch.object(tasks, "update_job_status"), \
                 mock.patch.object(tasks, "save_meeting") as save_meeting_mock:
                output_path = tasks.process_audio_task(
                    job_id="failed-localized-repair-job",
                    audio_path=audio_path,
                    output_dir=root / "output",
                    force_segment_indices=[1],
                    transcript_reuse_source_path=source_path,
            ...
            self.assertIsNotNone(output_path)
            repair_mock.assert_called_once()
            full_rerun_mock.assert_called_once()
            output_text = output_path.read_text(encoding="utf-8")
            self.assertIn("重新轉錄的第二段", output_text)
            self.assertNotIn("舊稿已確認有遺漏", output_text)
            quality_report = save_meeting_mock.call_args.kwargs["quality_report"]
            self.assertEqual(quality_report["segments"][1]["status"], "rerun")

    def test_delivery_gate_blocks_confirmed_transcript_faults_only(self):
        from backend import tasks
        ...
        unsafe = [{
            "index": 1,
            "issues": [
                "音訊含持續語音但時間戳在 11:00 至 13:00 間隔 120 秒",
                "分段疑似重複轉錄幻覺（連續重複 12 句，重複比例 90%）",
            ],
        }]
    ...
    def test_single_cached_transcript_with_confirmed_gap_is_retranscribed(self):
        ...
        summary_payload = {
            "discussion_summary": [{"topic": "單段快取重轉", "summary": "完成", "evidence_timecodes": ["00:00"]}],
            "final_decisions": [],
            "action_items": [],
        }
    ...
    def test_confirmed_segment_gap_blocks_summary_and_database_write(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}), \
                 ...
                 mock.patch.object(tasks, "_split_audio_to_segments", return_value=slices), \
                 mock.patch.object(tasks, "_load_segment_transcript_cache", return_value=None), \
                 mock.patch.object(tasks, "_transcribe_segment_with_recovery", side_effect=lambda *_args, **_kwargs: next(transcripts)), \
                 mock.patch.object(
                     tasks,
                     "_segment_transcript_current_quality_issues",
                     side_effect=[[], ["音訊含持續語音但時間戳在 11:00 至 13:00 間隔 120 秒"]],
                 ), \
    ...
    def test_meeting_detail_infers_repeated_transcript_segment_from_timestamps(self):
        ...
        record = {
            ...
            "title": "無分段標題重複舊紀錄",
            "date": "2026/07/08",
            "source_audio": "timestamp-only.webm",
            "output_path": "timestamp-only.md",
            "summary": "摘要",
            "job_id": None,
            "quality_score": None,
            "quality_label": None,
    ...
    def test_web_ui_shows_quality_report_and_segment_rerun_controls(self):
        ...
        self.assertIn("addIndex(match[1], text.slice(match.index, match.index + 80));", html)
        self.assertIn("const warningTimeRangeMatcher = /(?:重複時間|重複時段|問題時間|異常時間)", html)
        self.assertIn("const addTimeRangeTarget = (startSeconds, endSeconds = startSeconds) => {", html)
        self.assertIn("const firstIndex = Math.floor(startSeconds / REVIEW_SEGMENT_SECONDS);", html)
        self.assertIn("const lastIndex = Math.max(firstIndex, Math.floor(inclusiveEnd / REVIEW_SEGMENT_SECONDS));", html)
        self.assertIn("for (let index = firstIndex; index <= lastIndex; index += 1)", html)
        ...
        self.assertIn("function rerunReviewSegmentsFromCard", html)
        ...
        self.assertIn("await rerunMeeting(id, segmentIndices, false, false, triggerButtonId);", html)
        ...
        self.assertIn('aria-label="開啟會議 ${escapeHtml(r.title)}"', html)
        self.assertIn('aria-label="刪除會議記錄 ${escapeHtml(r.title)}"', html)
        ...
        self.assertIn("function contextualCardWarningTypeLabel(defaultLabel = '品質警示', qualityType = 'all')", html)
        ...
        self.assertIn("const defaultWarningType = cardWarningTypeLabel(warningPreview, reviewSegmentDetails.length > 0, warningText);", html)
        self.assertIn("const warningType = contextualCardWarningTypeLabel(defaultWarningType, filterType);", html)
        ...
        self.assertIn("const warningChipText = hasReviewContext && reviewSegments.length", html)
        self.assertIn("${warningType} ${reviewSegments.length} 段", html)
        self.assertIn("const warningChipTitle = hasReviewContext && reviewSegments.length", html)
        ...
        self.assertIn("function conciseReviewIssueLabel", html)
        self.assertIn("{ concise: true }", html)
        ...
        self.assertIn("groupedReasonMap.get(issue).push(display);", html)
        ...
        self.assertIn("const visibleReason = filteredWarningReason || cardReviewSegmentSummary || reviewSegmentReasons[0] || warningPreview;", html)
        ...
        self.assertIn("問題分段：${escapeHtml(reviewSegmentTitles.join('、'))}", html)
        ...
        self.assertIn('aria-label="關閉摘要編輯視窗"', html)
        self.assertIn('aria-label="關閉逐字稿編輯視窗"', html)
        self.assertIn('aria-label="關閉版本紀錄"', html)
        ...
        self.assertIn(".source-media-actions", html)
        self.assertIn(".source-media-mode-action[aria-pressed=\"true\"]", html)
        self.assertIn(".source-media-mode-pill", html)
        ...
        self.assertIn("function normalizeMediaKind", html)
        self.assertIn("function sourceMediaExplicitKind", html)
        ...
        self.assertIn("const previousPlayback = mediaPlayerPlaybackSnapshot(document.getElementById('source-media-player'));", html)
        self.assertIn("restoreMediaPlayerPlayback(document.getElementById('source-media-player'), previousPlayback);", html)
        self.assertIn("function syncActiveTimecode", html)
        self.assertIn("function setTimecodeButtonState", html)
        self.assertIn("function applyPendingSourceMediaSeek", html)
        self.assertIn("function cueSourceMedia", html)
        self.assertIn("player.dataset.pendingSeekSeconds = String(targetSeconds);", html)
        self.assertIn("delete player.dataset.pendingSeekSeconds;", html)
        self.assertIn("buttonSeconds <= currentSeconds + 0.75", html)
        self.assertIn("buttons.forEach(button => setTimecodeButtonState(button, button === activeButton));", html)
        self.assertIn("button.toggleAttribute('aria-current', active);", html)
        self.assertIn("button.setAttribute('aria-current', 'time');", html)
        self.assertIn("目前播放位置", html)
        self.assertIn("跳到此時間", html)
        self.assertIn("button.setAttribute('aria-label', `${raw}，跳到此時間`);", html)
        self.assertIn("syncActiveTimecode(targetSeconds);", html)
        self.assertIn("syncActiveTimecode(target);", html)
        self.assertIn("const VIDEO_FILE_EXTENSIONS = new Set", html)
        self.assertIn("const BROWSER_VIDEO_PREVIEW_EXTENSIONS = new Set", html)
        self.assertIn("const AUDIO_FILE_EXTENSIONS = new Set", html)
        self.assertIn("recording.source_media_type", html)
        self.assertIn("recording.mime_type", html)
        self.assertIn("recording.has_video === true", html)
        ...
        self.assertIn('id="source-media-player-status" class="source-media-player-status" role="status" aria-live="polite" aria-atomic="true" aria-busy="false"', html)
        self.assertIn("🎥 影片", html)
        self.assertIn("🔊 音訊", html)
        self.assertIn("source-media-mode-action", html)
        ...
        self.assertIn("setEditorSaveStatus('summary-editor-status')", html)
        self.assertIn("setEditorSaveStatus('transcript-editor-status')", html)
        self.assertIn("setEditorSaveStatus('summary-editor-status', '正在儲存版本...', 'loading', true);", html)
        self.assertIn("setEditorSaveStatus('transcript-editor-status', '正在儲存版本...', 'loading', true);", html)
        self.assertIn("setEditorSaveStatus('summary-editor-status', '已儲存版本。', 'success');", html)
        self.assertIn("setEditorSaveStatus('transcript-editor-status', '已儲存版本。', 'success');", html)
        self.assertIn("button.setAttribute('aria-busy', 'true');", html)
        self.assertIn("button.setAttribute('aria-busy', 'false');", html)
        self.assertIn(".editor-save-status.loading", html)
...
const reasonMatch = sandbox.result.match(/<div class="card-review-reason"[^>]*>([^<]+)<\\/div>/);
const visibleReason = reasonMatch ? reasonMatch[1] : '';
if (!visibleReason.includes('需複核：第 1 段 09:59-10:00、第 2 段 10:00-10:02：疑似連續重複轉錄（09:59-10:02）')) {{
  console.error(sandbox.result);
  process.exit(15);
}}
if (visibleReason.includes('重複時間：')) {{
  console.error(sandbox.result);
  process.exit(16);
}}
...
if (!sandbox.result.includes('title="偵測到可能的爆音；原始媒體檔已保留，重要內容請抽查。\\n第 8 段 70:00-80:00：疑似連續重複轉錄；重複時間：70:01-73:00"')) {{
  console.error(sandbox.result);
  process.exit(20);
}}
...
if (!sandbox.recordingResult.includes('錄音警示 1')) {{
  console.error(sandbox.recordingResult);
  process.exit(18);
}}
if (sandbox.recordingResult.includes('多重警示 2')) {{
  console.error(sandbox.recordingResult);
  process.exit(19);
}}
...
if (!sandbox.rerunnableResult.includes('可重跑分段：第 8 段 70:00-80:00：疑似連續重複轉錄（70:01-73:00）')) {{
...
const visibleIssueMatch = sandbox.unavailable.match(/<span class="quality-review-segment-issue">([^<]+)<\\/span>/);
const visibleIssueText = visibleIssueMatch ? visibleIssueMatch[1] : '';
if (!visibleIssueText.includes('100:00-110:00 · 疑似連續重複轉錄；同一句連續重複 22 次（103:04-105:14）（另 1 項）')) {{
  console.error(sandbox.unavailable);
  process.exit(20);
}}
if (visibleIssueText.includes('台語這樣比較好這樣比較好') || visibleIssueText.includes('非最後分段含自動過濾')) {{
  console.error(sandbox.unavailable);
  process.exit(21);
}}
