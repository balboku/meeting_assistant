...
- 每個逐字稿分段都有「重跑」按鈕。若品質檢查已定位到原始媒體仍有聲音的時間缺口，會先只轉錄該缺口並安全合併回既有逐字稿；局部補救無法通過檢查時才改用整段穩定重跑。其餘分段直接沿用原會議紀錄中的逐字稿；舊紀錄也會從分段標題自動重建按鈕。
- 「重新檢核」只用既有逐字稿與原始媒體檔重新計算問題分段，不呼叫 Gemini、不產生新會議。已成功補救的舊歷程會保留為註記，只有目前仍有音訊佐證的缺口會列為可重跑問題。
